  ...
  return (
    <div className={`group relative h-8 w-full flex items-center justify-center my-1 z-10 transition-opacity duration-200 ${visible ? 'opacity-100' : 'opacity-0 hover:opacity-100'}`}>
      ...
      <div className="flex gap-2 relative bg-transparent">
        <button 
          onClick={onAddCode}
          className="flex items-center gap-1.5 bg-sim-surface border border-sim-border text-xs font-mono font-medium text-sim-muted px-3 py-1.5 rounded hover:bg-sim-border hover:text-white hover:border-sim-muted transition-all transform hover:scale-105 active:scale-95"
        >
